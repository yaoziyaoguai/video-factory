...
每张候选卡会解释总分的主要贡献项和风险扣分，说明为什么适合做成视频，并给出开场、展开、收束三拍视觉计划。模板目前不参与新制作，也不会覆盖用户明确要求。不满足受众收益、两秒钩子、来源可靠性、可拍性或视频价值的候选只能保留为待补充线索，规则保底不能冒充可开拍的模型选题。素材来源只是建议，可在正式 Brief 中替换为本地素材、素材库或已配置的生成式视觉服务。`editorial_card` 只在导演主动把它选为正式创作内容时可用，不能承担失败降级。
...
机会按最终分和新鲜度排序。选择后检查证据 URL、hook、受众、分数解释和三拍视觉计划；右侧决策面板会显示五项制作能力是否就绪。点击“新建制作”后，标题、hook、受众、系列、平台和已保存的经济型制作默认值会预填到正式 Brief，但仍可人工编辑，只有点击“开始制作”才会派发 run。热点入口还会保存实际读取状态、原文 URL、内容摘要和可引用段落；没有正文、登录页或读取失败会明确显示，不能把标题线索写成“已读原文”。

制作弹窗的 Voice Studio 会读取可用声音，并提供按内容校准的声音方案；高级设置仍可试听并调整语速、标点停顿和声音质感。所选音色与配音能力会在客户端和服务端交叉校验。在“创作设置”保存画面来源策略、导演角色、平台、时长、终审方式、声音和默认画面来源后，新制作会自动继承；创建时仍可针对单条视频修改。模型按能力分组，用户选择的模型优先，其余健康且合同兼容的模型只在连接、超时、限流、容量或明确不可用时依次接管。

付费图片和视频只按导演实际选中的镜头逐项报价，不设置全片费用上限。拒绝报价不会自动再花钱；可以填写低于当前报价的降本目标，并由创作者明确触发重新规划，之后再审核新方案和新报价。TTS 自动执行并后台记账，不进入图片/视频报价弹窗。

### 4. 讨论并确认创作方案

开始制作后，系统依次停在“导演初案”“脚本”“分镜与画面方案”。每一步都展示当前完整稿件，并允许用自然语言询问原因、比较另一版、要求修改、采用备选或撤销本轮；讨论轮数不作为产品限制。系统不会把普通消息中的“同意”当作确认，也不会因刷新或自动轮询覆盖尚未发送的输入。

“确认当前版本”是单独动作：系统对用户当前看见的版本做一次独立复核，通过后才进入下一角色；不通过时保留稿件和具体意见继续讨论。只有最终分镜确认后才编译正式执行方案并进入图片/视频报价。修改上游要求、角色模型、声音节奏或素材能力时，只让受影响阶段及下游重新确认，已成立且仍兼容的稿件和素材继续保留。

### 5. 跟踪生产

“制作记录”从磁盘读取 run，可以按重点、制作中、待处理、已结束和归档查看，也可以按标题搜索、批量归档或恢复。制作详情页通过 SSE 接收 checkpoint；连接中断时页面会提示并由浏览器自动重连。模型步骤完成后可展开“这一步为什么用了这些时间”，区分内容生成、创作讨论、确认时独立复核、结构校验、已证实调用与状态未知的调用；查询、刷新和人工等待不算模型执行，也不会把缺失的时间数据补成 0。
...
3. 最终视觉审片必须完成 GLM/Codex 同证据独立双审，不会因一个分支暂时不可用而降级成单审。报告默认先展示 5 条重要问题，其余可展开查看。
4. 视觉审片建议修改时，可点击 finding 跳到对应时间点，选择更早且已物化的非说明卡母片，并填写原因后提交局部返修。
5. 局部返修保留配音和未受影响素材，只按 finding 的主责任节点与受影响下游重做，不无条件扩大到全片；单纯复用已有母片不会创建新的素材 Provider 调用或费用授权。
6. 批准会继续生成发布包；终态打回必须填写明确原因。
7. 决定、素材版本和 revision 持久化到 `run.json`，重复/冲突决定返回 `409`。

终态打回后可以“调整方案后重新制作”。新制作默认继承当时的模型、画面来源、声音和导演配置；人工打回原因与视觉 finding 会按脚本、导演方案和画面素材分组预填，并作为真实节点输入传给下一轮角色，而不是只显示为备注。历史模板快照只用于查看，不会进入新返工。

### 7. 检查总配置与制作复盘

“创作设置”集中设置新建制作默认值，并展示热点服务、声音、画面、岗位模型、来源授权和发布渠道的真实状态；API Key 仍只存在服务端环境中，不会回传浏览器。素材来源与授权默认按视频折叠整理，也可按资产查看；历史固定技术说明只在展示层转成中文，持久化 provenance 原文保持不变。“模板工坊”继续支持新建、克隆、编辑草稿、发布版本、删除和恢复，供资料管理与历史查看；当前模板不会被新制作、重新规划或返工读取为执行要求。“制作复盘”目前只统计 persisted run 的制作结果；平台播放、完播、互动和涨粉在连接数据前不会显示，也暂不支持 Web 手工录入。

费用复盘中，“报价授权金额”只表示用户允许执行的上限，不是消费；“已记录费用”才是工作流账本。`configured_rate` 会显示为“按配置费率记录 · 非服务商确认账单”，只有 `provider_reported` 才表示服务商回传费用。
...
| AI 选题总编 | 已实现 | 宿主机模型 bridge；结构化输出；失败时规则结果仅作待补充线索，不能绕过开拍准入 |
...
| 创作默认值 | 已实现 | 画面来源策略、导演、平台、时长、终审、声音与素材服务持久化并自动继承 |
| 模板工坊 | 暂停参与生产 | 可新建、克隆、编辑草稿、发布版本、删除、恢复及查看历史快照；新制作、重新规划和返工不应用模板 |
| 素材来源与授权 | 已实现 | 默认按视频折叠，可切换按资产；保留来源、授权状态与未完成付费任务的保守恢复记录 |
...
| 模型语义层 | 已实现 | 选题、系列、编剧、参考语法、视觉导演、选片、视觉审片与发行角色；首选/替补按故障策略切换，人工终审与确定性技术质检保留为硬边界 |
...
| 审片与发布包 | 已实现 | 视频 Range 播放、技术报告、finding 定位、同 run 局部素材返修、批准/打回、发布包 |
...
- `POST /api/runs`
- `GET /api/runs/:runId`
- `GET /api/runs/:runId/creative-review`
- `POST /api/runs/:runId/creative-review/commands`
- `GET /api/runs/:runId/creative-review/commands/:commandId`
