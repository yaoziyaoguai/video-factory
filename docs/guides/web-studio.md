...
3. 视觉审片建议修改时，可点击 finding 跳到对应时间点，选择更早且已物化的非说明卡母片，并填写原因后提交局部返修。
4. 局部返修保留配音和未受影响素材，只重跑渲染、技术质检、视觉审片与人工终审；不会创建新的素材 Provider 调用或费用授权。
5. 批准会继续生成发布包；终态打回必须填写明确原因。
6. 决定、素材版本和 revision 持久化到 `run.json`，重复/冲突决定返回 `409`。
