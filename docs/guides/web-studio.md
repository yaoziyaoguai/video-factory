...
制作弹窗的 Voice Studio 会读取本机声音，支持试听、语速、标点停顿和 `natural/intimate/social` 三种声音质感。安装 Kokoro 后可切换到本地神经语音；未安装时可以选择 macOS 中文语音。所选音色与配音能力会在客户端和服务端交叉校验。在“素材与模型”保存声音、素材和制作方案后，新制作会自动继承；按钮显示“已是制作默认”时不需要重复保存。
...
“素材与模型”展示当前机器的能力状态，并单独暴露趋势采集、推理模型和生成式视觉能力缺口。“制作复盘”目前只统计 persisted run 的制作结果；平台播放、完播、互动和涨粉在连接数据前不会显示，也暂不支持 Web 手工录入。
