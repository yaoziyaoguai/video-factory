...
const SCREENWRITER_DIRECTIVE = [
  ...
  "输入含 rework 时，previousScript 是上一版基线，instruction 是本次人工确认的修改要求；保留未被要求修改的叙事与事实，只重写要求涉及的内容，并输出一份可直接继续生产的完整脚本。",
...
const DIRECTOR_PLAN_DIRECTIVE = [
  ...
  "输入含 rework 时，previousDirectorPlan 是上一版基线，visualDirectionInstruction 与 assetInstruction 是本次人工确认的修改要求；必须把要求落实到对应的视觉圣经、逐镜路由和生成提示字段，保留未受影响镜头，并输出完整可执行方案。",
