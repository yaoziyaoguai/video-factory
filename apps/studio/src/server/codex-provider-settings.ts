...
export async function readCodexProviderSettings(
  ...
  const probe = options.socketProbe ?? probeCodexSocket;
...
async function probeCodexSocket(socketPath: string): Promise<CodexSocketStatus> {
  let stats: Stats;
  try {
  ...
  return probeCodexHealth(socketPath);
}
...
function probeCodexHealth(socketPath: string): Promise<CodexSocketStatus> {
    ...
    const request = http.request({
      ...
      response.on("end", () => {
        ...
        try {
          ...
          settle(body.protocolVersion === CODEX_BRIDGE_PROTOCOL_VERSION ? "ready" : "protocol_mismatch");
        } catch {
          settle("protocol_mismatch");
        }
      });
