...
describe("JsonTemplateStore", () => {
  ...
  it("migrates the known legacy acceptance templates out of the production catalog", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "video-factory-templates-"));
    const file = path.join(root, "templates.json");
    await writeFile(file, JSON.stringify({
      ...
      templates: [
        ...
        {
          ...structuredClone(BUILTIN_TEMPLATES[0]!),
          id: "ranked-comparison-copy-72c5140c",
          name: "榜单对比 QA 副本",
          description: "用于验证模板编辑、模型覆盖与草稿保存，不用于正式生产。",
        },
        {
          ...structuredClone(BUILTIN_TEMPLATES[0]!),
          id: "custom-7b28591c-e3d",
          name: "自定义模板 QA 验收",
          description: "验证用户从空白创建模板、编辑结构与保存草稿。",
        },
        {
          ...structuredClone(BUILTIN_TEMPLATES[0]!),
          id: "creator-qa-notes",
