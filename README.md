...
- 最终成片必须由 GLM 与 Codex 对同一证据快照独立双审。两个分支必须是不同的实际 Provider 和模型，任一分支缺失或落到同一模型都只能保留已完成结果并继续未完成分支，不能降级成单审发布。
- 模型节点保存一个首选模型，同能力下其余健康模型作为有序候选；只在连接、超时、限流、容量或模型不可用时切换，结构/业务/审计失败不会被替补模型掩盖。
